...
Explain how the protocol handles off‑chain encrypted storage, access control via signed keys/receipts and auditability without exposing raw data.  Outline data quality scoring, deduplication and regulatory positioning: segment‑level aggregation, no resale of raw personal data and buyer‑specific access rights that respect privacy laws.

## 9. Protocol Architecture & Smart Contracts

Describe the on‑chain components: the PAT token, segment registry, market contracts, rewards distributor and staking/bonding mechanisms.  Summarize off‑chain components: ingestion pipelines that generate segments, scoring and decay engines, access enforcement and metadata storage.  Discuss governance and upgradability principles to ensure long‑term adaptability.
